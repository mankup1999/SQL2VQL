...
ANALYTIC EXAMPLE-
SELECT last_name, salary, department_id,PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY salary DESC) OVER (PARTITION BY department_id) "Percentile_Cont" FROM employees WHERE department_id IN (30, 60);
*/
...
public class PERCENTILE_CONT
{
	...
	{
		query=query .replaceAll("(?i) over "," OVER ")
					.replaceAll("(?i)percentile_cont","PERCENTILE_CONT")
		...
		int index_per_cont=query.indexOf("PERCENTILE_CONT");
		...
		int index_over=perc_cont.indexOf(" OVER ");
		...
		{
			int index_partition=query.indexOf("PARTITION BY")+12;
			int index_partition_ends=query.indexOf(")",index_partition+1);
			group_attr=query.substring(index_partition,index_partition_ends).trim();
		}
		...
		if(index_over==-1)
			...
			as_var=perc_cont.substring(perc_cont.indexOf(")",perc_cont.indexOf("PARTITION BY")+1)+1).trim();
		String x="";
		if(as_var.length()>0)
		...
		if(index_over==-1)
			...
			result+="CREATE VIEW v1 AS \n\tSELECT "+group_attr+", COLLECT("+order_attr+")"+" AS "+order_attr+x+" FROM "+table+" GROUP BY "+group_attr+" ORDER BY "+order_clause+","+group_attr+" "+last+"\n";
		...
		result+="\t\t\tDECLARE val NUMBER :=(val1 + val2)/2;\n";
		...
		if(index_over!=-1)
			result+="\n\nCREATE VIEW v3 AS \n\tSELECT "+group_attr+", "+order_attr+" AS "+order_attr+x+" FROM "+table+" ORDER BY "+order_clause+","+group_attr+" "+last+"\n";
		...
		if(index_over==-1)
			...
			result+="FROM "+"v3"+" INNER JOIN temporary ON "+"v3"+"."+group_attr+" = "+"temporary."+group_attr+" "+last+"\n";
