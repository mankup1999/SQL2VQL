...
public class JOIN_CONVERSION
{
	public static ArrayList<String> HighlightedText=new ArrayList<>();
	public static ArrayList<String> HighlightedTextSQL=new ArrayList<>();
	public static String[] SQL;
	public static String[] VQL;
	public static void main(String[] xyz)
	{
		...
		String SqlQuery=kb.nextLine();

		ArrayList<String> LinesOfQuery=new ArrayList<>();
		int len_LinesOfQuery=0;
		int prev=0;
		int index_for_line=SqlQuery.indexOf("\\n");
		
		while(index_for_line!=-1)
		{
			LinesOfQuery.add(SqlQuery.substring(prev,index_for_line));
			len_LinesOfQuery++;
			prev=index_for_line+2;
			index_for_line=SqlQuery.indexOf("\\n",prev);
		}
		LinesOfQuery.add(SqlQuery.substring(prev));
		len_LinesOfQuery++;
		
		int index_for_lines=0;
		//Put the SQL query into 'SqlQuery' variable
		
		
		boolean flag=false;
	    while(index_for_lines<len_LinesOfQuery)
	    {
	        Line=(LinesOfQuery.get(index_for_lines++)).trim();
		...
		try
		{
			...
			//System.out.println(HighlightedTextSQL);
			SQL=HighlightedTextSQL.toArray(new String[HighlightedTextSQL.size()]);
			VQL=HighlightedText.toArray(new String[HighlightedText.size()]);
			/*System.out.println("-------SQL(To be Highlighted)-----------");
			for(int i=0;i<SQL.length;i++)
				System.out.println(SQL[i]);
			System.out.println("-------VQL(To be Highlighted)-----------");
			for(int i=0;i<VQL.length;i++)
				System.out.println(VQL[i]);*/
	...
	public static String convert(String query)
	{
		...
		*/
		...
		if(index_where!=-1)
		{
			HighlightedTextSQL.add(from_to_where);
			...
			for(int i=0;i<len_con_for_joins;i++)
				HighlightedTextSQL.add(cond_for_joins[i]);
			...
			if(len_con_for_joins>0)
			{
				...
				for(int i=1;i<len_tables;i++)
				{
					...
					addToList+=join_type+tables[i];
					String temp=findJoinCond(s2,already_tables,cond_for_joins,len_con_for_joins);
					result+=temp;
					addToList+=temp;
					already_tables+=" "+s2;
				}
